...
def preprocess(dataset, fileName): 
    ...
    for i in range(samples):
        ...
        for j in range(time_samples - 40 + 1):
            ...
            # Augment data
            for k in range(1):
                dataset.data[dataset.data_idx] =  torch.unsqueeze(subrange, dim=0)
                dataset.data_idx+=1
...
class MixedDataset(Dataset):
    def __init__(self, files, dir = "data/capstone/dances/", transform=None, target_transform=None):
